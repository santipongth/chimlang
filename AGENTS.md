...
4. brief ของเฟสปัจจุบัน (ดู "Scope ปัจจุบัน" ใน `CLAUDE.md` — ล่าสุด: `docs/PHASE8-BRIEF.md`) — backlog + checklist
...
- **Cap ต่อ run = 1,000 agents** (ผู้ใช้สั่งขยายจาก 10 เมื่อ 6 ก.ค. 2026 พร้อมเริ่ม Phase 3) — deep 5,000 ต้องขออนุมัติก่อน; `RUN_BUDGET_USD_CAP=5`/run + งบ $50/เดือน ยังบังคับผ่าน BudgetGuard ทุก run
...
- Scope = เฟสปัจจุบันตาม `CLAUDE.md` เท่านั้น (ล่าสุด: Phase 8 ตามมติผู้ใช้ 15 ก.ค. 2026) — ห้ามเริ่มงานนอก scope โดยไม่ถาม
...
2. ติ๊ก checklist ใน brief ของเฟสปัจจุบัน (`docs/PHASE<N>-BRIEF.md`) ตามที่คืบจริง
3. ตัดสินใจเทคนิคใหม่/พลิกของเดิม → เขียน `docs/adr/ADR-XXXX-*.md`
4. **บันทึกกิจกรรม session ลง `docs/journal/YYYY-MM-DD-session.md`** (คำสั่งผู้ใช้ 12 ก.ค. 2569):
   เก็บ "สิ่งที่คุยกัน คำอธิบาย/เหตุผลที่ให้ผู้ใช้ มติผู้ใช้ และงานที่ทำ" — ละเอียดกว่า STATE
   (STATE = สถานะ/ผลลัพธ์, journal = บริบท/เหตุผล); วันเดียวหลาย session ให้ append ต่อท้ายไฟล์เดิม
5. commit ทุกอย่าง (ข้อความ commit ภาษาไทย อธิบาย "ทำไม" ไม่ใช่แค่ "ทำอะไร") — working tree ต้องสะอาด
