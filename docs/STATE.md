...
4. ยืนยันสุขภาพระบบ: `uv run pytest -q` (ต้องได้ **253 passed**) ก่อนเริ่มงานใหม่
...
- test: **253 ข้อเขียว** | ต้นทุนสะสม ~$0.62 (Phase 5 ทั้งเฟส $0 — กลไกล้วน ไม่มี LLM call) | benchmark page: docs/reports/public-benchmark.md (rebuild ด้วย `scripts/build_benchmark_page.py` หลัง hindcast/resolve ใหม่ทุกครั้ง)
