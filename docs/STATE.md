...
- **ใหม่ (18 ก.ค. ถอด Watchlist + แปลไทย/อังกฤษเต็มรูปทั้ง UI):** (1) **ADR-0024** ถอด Watchlist
  ...
  `2026-07-18-remove-watchlists-v1` ลบตาราง watchlists (360 แถว — ทั้งหมดเป็น test data)
  + alerts (180 แถว cascade); governance ledgers ไม่ถูกแตะ. (2) **Localization sweep เต็มรูป
  ตามมติผู้ใช้** ("แปลไทยละเอียด คนทั่วไปเข้าใจ ศัพท์เทคนิควงเล็บอังกฤษ"): เกลา dictionary 256 key
  + เพิ่มใหม่ ~106 key + scan ทุกหน้า ย้าย string ที่ hardcode (~85 จุดใน 10 ไฟล์ หนักสุด RunDetail
  ~45 จุด) เข้า `t()` ครบ; เพิ่ม `hint_en` ให้ LLM providers (backend) แก้การ์ด provider ที่โชว์ไทย
  ในโหมดอังกฤษ; สตริง en ทุกตัวไม่มีอักษรไทย (E2E ตรวจ). Verification: backend 409 tests เขียว,
  Ruff, migration no-op, OpenAPI regenerate, build, Vitest 11, Playwright 20/20, Compose rebuild
  + live (/watchlists=404, app=200) ผ่านครบ.

- **ใหม่ (18 ก.ค. News Desk upgrade — 4 ข้อตามมติผู้ใช้):** (1) RSS parser อ่าน timestamp จริง
  (pubDate/published/updated/dc:date ทั้ง RFC822+ISO) เก็บลง provenance (`news_items.published_at`,
  migration `2026-07-18-news-published-at-v1`) และกรองข่าวเก่ากว่า max age (default 14 วัน;
  ไม่ทราบวันที่ = คงไว้) — Tavily คงโหมด general ตามมติ (ต้องการข้อมูลทั่วไปตามคำค้น ไม่ใช่ข่าวอย่างเดียว),
  (2) RSS จัดอันดับด้วย Thai BM25 กับหัวข้อ run แล้วเลือก top-k แทน "10 รายการแรก" (reuse
  `_bm25_scores` จาก sources.py; รายการ blocked/error ยังเข้า snapshot เป็น audit เช่นเดิม),
  (3) cache TTL + max age ตั้งจากหน้า Settings ได้ (`news_cache_ttl_hours` 0/1–168,
  `news_max_age_days` 0/1–365; 0 = ใช้ .env default 6 ชม./14 วัน — pattern เดียวกับ synthesis tokens),
  (4) ตัดข่าวซ้ำแบบใกล้เคียงด้วย 3-gram **containment coefficient** > 0.7 (เลือก containment แทน
  Jaccard เพราะข่าวเรื่องเดียวกันคนละสำนัก = เนื้อเดิม+พาดหัวต่างกัน ซึ่ง Jaccard เจือจางจนหลุดเกณฑ์ —
  test พิสูจน์แล้ว). Verification: newsdesk 22 tests (ใหม่ 6), phase6+jobs เขียว, Ruff, build,
  Vitest 11, Compose rebuild + live PUT/422/reset ของ settings ใหม่ผ่านจริง; ระวัง: test ที่มี queries
  ต้อง stub `effective_news_config` กัน Tavily จริง (เจอ live call หลุดใน test ระหว่างพัฒนา — แก้แล้ว).

- **ใหม่ (18 ก.ค. decommission รอบสอง — ADR-0023):** ผู้ใช้สั่งลบ 6 รายการพร้อมโค้ด/DB:
  (1) ข้อความ engine ภายนอก/MiroFish ใน wizard, (2) vector/RRF retrieval ทั้งเส้นทาง —
  retrieval เหลือ Thai BM25+lexical, ลบ embedding adapter/settings/UI fields และ migration
  `2026-07-18-remove-vector-retrieval-v1` ลบตาราง `run_chunk_embeddings` (ยืนยัน 0 แถวก่อนลบ),
  (3) Run-local reflection ทั้งฟีเจอร์ (ลบ simulation/reflection.py + wiring + toggle),
  (4) กล่องรับทราบ synthetic population ใน wizard/Experiments — run auto-freeze PopulationSetV1
  โดยไม่ต้องติ๊ก (contract `acknowledged_synthetic` ของ API สร้าง set ตรงยังอยู่),
  (5) กล่อง "เป้าหมายการตรวจสอบ" (claim/measurement/due days) — prediction อัตโนมัติยังครบกฎเหล็กข้อ 3,
  (6) banner "not a real poll" ใน Gallery + field disclaimer ใน /gallery.json —
  **watermark PDF/HTML export และ labels ใน watermark metadata คงเดิมตาม GOV-03 (กฎเหล็กข้อ 4)**.
  RunBody ตัด `retrieval_mode`/`reflection`/`population_acknowledged`. Verification: backend เขียวทั้ง suite,
  Vitest 11, Playwright 20, Ruff, OpenAPI/build, Compose rebuild + live checks (migration applied,
  run ไม่ต้องติ๊ก, settings/gallery ไม่มี field เดิม) ผ่านครบ.

- **ใหม่ (18 ก.ค. Core Engine v2 — ADR-0022):** audit+calibrate Fabric/Debate/Red Team ตามมติผู้ใช้
  (ไม่รัน LLM จริง; Fabric harness รันจริง $0). Harness ใหม่ `scripts/calibrate_fabric.py` จับบั๊กเชิง
  วิธีวิทยา: RNG เส้นเดียวทำให้คู่เทียบ A/B ปน noise (delta คำชี้แจงพลิกเป็น +0.04) → แก้เป็น
  **common random numbers** (hashed draw ต่อ seed/event/msg/agent) + re-exposure ≤3 ครั้ง
  (complex contagion) + WOM ring สุ่ม seeded + รวมค่าคงที่กลไกจุดเดียว. ผลหลังแก้: delta −0.096
  (n=100) / −0.099 (n=1000) ถูกทิศ+scale-invariant, FAB-01 latency ผ่าน, sensitivity ชี้
  `public_feed.trust` ต้อง calibrate ก่อนสุดเมื่อได้ข้อมูลจริง. Debate: feed แบบ selective exposure
  (weighted ตาม channel_mix), voice layer TRUST-07 (`voice_share`/`voice_population_stance_gap`),
  conformity metrics (`convergence_rate`/`stance_flips` แยก conforming vs evidenced/
  `consensus_warning`) และ **Red Team ยกระดับเป็น devil's advocate เต็มรูป** (โจมตี majority,
  contrarian cap จุดยืน≤0 + ห้าม concede, ฟีดเห็นโพสต์ align กับ majority, metrics
  `red_team_pressure`) ตามงานวิจัย "soft framing ไม่ได้ผล". Verification: engine tests เดิมผ่านครบ
  โดยไม่แก้เกณฑ์, debate tests 22 (ใหม่ 3), Ruff, live Fabric smoke complete $0. รายงาน:
  docs/reports/core-engine-audit-2026-07.md; **Debate metrics ใหม่รอผู้ใช้รัน LLM จริงเพื่ออ่านผล**.
- **ใหม่ (18 ก.ค. ADR-0021 — synthesis token ตั้งได้จาก UI + นโยบายทดสอบ pilot):** ผู้ใช้สั่ง
  (1) เพดาน token ของ Executive Readout ตั้งจากหน้า Settings ได้ — เพิ่ม `llm_synthesis_max_tokens`
  (0 = default 2,000; ช่วง 500–16,000; retry ได้ 1.5 เท่าอัตโนมัติ) ผ่าน app_settings overlay แบบ
  ADR-0006 และ cost estimate/readiness ใช้ค่า effective เดียวกัน; live verify: PUT 4000 → มีผลจริง,
  ค่านอกช่วง → 422, reset 0 → กลับ default. (2) **mock/stubbed tests ไม่เป็น gate ระหว่าง pilot** —
  CI สอง job แรกเป็น continue-on-error (informational), gate ที่ block เหลือ live integration เท่านั้น;
  ผู้ใช้ทดสอบจริงเองและจะแจ้งปัญหา ไม่ต้องทำ calibration เพิ่ม; แก้ถ้อยคำใน AGENTS.md/CLAUDE.md แล้ว
  (ยังเขียน unit test คู่ module ใหม่และรัน targeted suite ก่อน commit เพื่อจับ regression).
...
- 2026-07-18 (Claude Fable 5): **ถอด Watchlist (ADR-0024) + localization sweep เต็มรูป** —
  ลบ 8 ไฟล์ แก้ ~27 ไฟล์, drop ตาราง watchlists/alerts (test data ล้วน 360/180 แถว ตรวจก่อนลบ),
  เกลา/เพิ่มคำแปลรวม ~360 key และย้าย hardcode strings ทุกหน้าเข้า dictionary; งานหลักทำผ่าน
  subagent 3 ตัวแล้วตรวจ/รวม/ยืนยันที่ session หลัก. Backend 409, Vitest 11, Playwright 20,
  Ruff, migrations no-op, live check ผ่าน. ไม่แตะ `.gitignore`/`diagrams/` ของผู้ใช้.

- 2026-07-18 (Claude Fable 5): **News Desk upgrade 4 ข้อ** — published_at provenance + age filter,
  BM25 top-k ที่ขา ingest, TTL/max-age ตั้งจาก Settings, near-dup containment 0.7; migration ใหม่
  1 ตัว, tests ใหม่ 6, live verify settings ผ่าน; Tavily คง general search ตามมติผู้ใช้.
  ไม่แตะ `.gitignore`/`diagrams/` ของผู้ใช้.

- 2026-07-18 (Claude Fable 5): **ADR-0023: ถอด vector retrieval/reflection/กล่องรับทราบ/เป้าหมาย
  ตรวจสอบ/banner + ข้อความ MiroFish** — ลบทั้ง UI/API/engine/DB ตามคำสั่งผู้ใช้; retrieval เหลือ
  BM25 deterministic, RunBody บางลง 3 field, ตาราง embeddings ถูก drop (0 แถว), watermark ฝั่ง
  export คงเดิมตามกฎเหล็ก. Backend full suite เขียว, Vitest 11, Playwright 20 (แก้ spec ที่กด
  checkbox ที่ถูกถอด), live verify บน Compose ครบ. ไม่แตะ `.gitignore`/`diagrams/` ของผู้ใช้.

- 2026-07-18 (Claude Fable 5): **Core Engine v2 (ADR-0022): audit+calibrate Fabric/Debate/Red Team** —
  harness ใหม่จับ RNG coupling ที่ทำให้ A/B delta ปน noise → CRN + re-exposure + WOM ring + ค่าคงที่
  รวมจุดเดียว; ผล calibration delta ถูกทิศ scale-invariant, sensitivity จัดอันดับพารามิเตอร์ที่ต้อง
  calibrate จริง. Debate เพิ่ม selective feed, voice layer, conformity/consensus metrics และ
  devil's advocate red team (ตามงานวิจัย IUI'24/arXiv — soft framing ไม่ได้ผล). Tests เดิมผ่านครบ
  ไม่แก้เกณฑ์, debate tests 22, live Fabric smoke $0. รายงาน core-engine-audit-2026-07.md;
  Debate ยังไม่ validate กับ LLM จริงตามมติ — ผู้ใช้รันเองแล้ว metrics ใหม่โชว์ใน payload อัตโนมัติ.
- 2026-07-18 (Claude Fable 5): **ADR-0021: synthesis max tokens ตั้งจาก Settings + ถอด mock test
  จากการเป็น gate** — เพิ่ม `llm_synthesis_max_tokens` ครบทั้ง config/appsettings (validate 0 หรือ
  500–16000)/userconfig overlay/debate engine/cost estimate/readiness/Settings API/UI + i18n;
  retry ceiling = max(base+500, base×1.5). CI: mocked+stubbed jobs เป็น continue-on-error,
  live integration เป็น gate เดียว; อัปเดต AGENTS.md/CLAUDE.md ตามมติ. Verification แบบ targeted:
  Ruff, pytest phase6+jobs 65 tests, OpenAPI regenerate, build, Compose rebuild แล้ว live PUT/422/reset
  ผ่านจริง; ค่าใน DB ถูก reset กลับ 0 (default) หลังทดสอบ. ไม่แตะ `.gitignore`/`diagrams/` ของผู้ใช้.
