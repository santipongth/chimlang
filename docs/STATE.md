...
- เฟส: **Phase 2 (Rehearsal & Signal) — เริ่ม 6 ก.ค. 2026 (ผู้ใช้ approve แผน) | P2-M1..M3 เสร็จ** — ดู docs/PHASE2-BRIEF.md (Phase 0+1 ปิดครบ, exit criteria ผ่าน)
...
- ถัดไป: **P2-M4 (Sim-to-Signal API + Out-of-Sample Harness — SIG-01..04)**: features พร้อมช่วง + metadata บังคับ (run id/fragility/calibration/provenance hash), GOV-02 ปิด signal ใน election (guard มีแล้ว ต่อเข้า endpoint), harness วัด IC/hit rate เทียบ baseline พร้อม train/test split; จากนั้น M5 living memory, M6 influence graph (ดู PHASE2-BRIEF)
