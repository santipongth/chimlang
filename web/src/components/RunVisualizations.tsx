...
function FallbackTable({
  ...
  return (
    <details className="mt-3 rounded-xl border border-border bg-background p-3 text-xs">
      {/* ข้อความไทย "ตารางข้อมูล:" ถูกตรึงโดย E2E (run-detail.spec) — ห้ามแก้ฝั่งไทย */}
      <summary className="cursor-pointer font-medium">{t("rv_data_table")}: {label}</summary>
...
export function UniverseRangeChart({ universes, fallbackRange }: { universes: UniverseDatum[]; fallbackRange?: [number, number] }) {
  ...
  const data = universes.length
    ? universes
    : fallbackRange
      ? [{ universe_id: 0, estimate: (fallbackRange[0] + fallbackRange[1]) / 2, ci95: fallbackRange, conclusion: t("rv_range_combined") }]
      : [];
  ...
  if (!data.length) return <p className="text-sm text-muted-foreground">{t("rv_no_multiverse")}</p>;
  return (
    <>
      <AccessibleChart option={option} label={t("rv_universe_aria")} testId="universe-range-chart" />
...
export function ScenarioComparisonChart({ scenarios, population }: { scenarios: ScenarioDatum[]; population: { segment: string; population_share: number }[] }) {
  ...
  if (!rows.length) return <p className="text-sm text-muted-foreground">{t("rv_no_scenario")}</p>;
  return (
    <>
      <AccessibleChart option={option} label={t("rv_scenario_aria")} height={Math.max(280, rows.length * 44)} testId="scenario-comparison-chart" />
...
export function StanceTimelineChart({ posts }: { posts: DebatePostItem[] }) {
  ...
  const option = useMemo<EChartsOption>(() => ({
    ...
    tooltip: { trigger: "item" },
    xAxis: { type: "value", min: 1, max: Math.max(1, rounds.length), interval: 1, name: t("rv_round_axis") },
  ...
  }), [averages, ok, rounds, t]);
  if (!ok.length) return <p className="text-sm text-muted-foreground">{t("rd_stance_no_data")}</p>;
  return (
    <>
      <AccessibleChart option={option} label={t("rv_stance_aria")} testId="stance-timeline-chart" />
...
export function StabilityMatrix({ report }: { report: ValidationReport }) {
  ...
  const option = useMemo<EChartsOption>(() => ({
    ...
    yAxis: { type: "category", data: [t("rv_run_result")] },
  ...
  return (
    <>
      <AccessibleChart option={option} label={t("rv_stability_aria")} height={220} testId="stability-matrix" />
...
export function ContentionGraph({ posts }: { posts: DebatePostItem[] }) {
  ...
  if (!rounds.length) return <p className="text-sm text-muted-foreground">{t("rv_no_contention")}</p>;
  return (
    <div>
      <div className="mb-3 flex flex-wrap items-center gap-2" aria-label={t("rv_filter_round_aria")}>
      ...
      <div ref={container} role="img" aria-label={`${t("rv_contention_round_aria")} ${round + 1}`} tabIndex={0} data-testid="contention-graph" className="h-[360px] w-full rounded-xl border border-border outline-none focus-visible:ring-2 focus-visible:ring-primary" />
      <div className="mt-3 flex flex-wrap gap-2" aria-label={t("rv_select_segment_aria")}>
      ...
      {graph.hiddenSegments > 0 && <p className="mt-2 text-xs text-muted-foreground">{t("rv_hidden_note").replace("{shown}", String(graph.nodes.length)).replace("{total}", String(graph.nodes.length + graph.hiddenSegments))}</p>}
...
export function EvidenceLineage({ sources, subject, posts, synthesis }: { sources: any[]; subject: string; posts: DebatePostItem[]; synthesis: string }) {
  const { t } = useLang();
  const sourceNodes = sources.slice(0, 8).map((s, i) => ({ name: `${t("rv_source_word")} ${i + 1}: ${String(s.label ?? s.title ?? s.provider ?? "evidence").slice(0, 28)}` }));
  const segments = [...new Set(posts.filter((p) => !p.failed).map((p) => p.segment))].slice(0, 10);
  const claim = `Claim: ${subject.slice(0, 42)}`;
  const result = `Synthesis: ${synthesis.slice(0, 42) || t("rv_synth_fallback")}`;
  ...
  if (!sourceNodes.length || !segments.length) return <p className="text-sm text-muted-foreground">{t("rv_no_lineage")}</p>;
  return (
    <>
      <AccessibleChart option={option} label={t("rv_lineage_aria")} height={420} testId="evidence-lineage-chart" />
