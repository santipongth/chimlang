...
export function VirtualizedDebateList({ posts }: { posts: DebatePostItem[] }) {
  ...
  return (
    <div
      role="list"
      aria-label={`Debate posts ${posts.length} รายการ`}
            ...
            >
              <div className="flex items-center justify-between gap-2 text-xs text-muted-foreground">
                <span className="flex min-w-0 flex-wrap items-center gap-2 font-medium text-foreground">
                  ...
                  <span className="rounded-full border border-border px-2 py-0.5 text-[10px] text-muted-foreground">{post.move_type ?? "legacy claim"}</span>
              ...
              <p className="mt-1 line-clamp-2">{post.failed ? `(agent response failed: ${post.failure_reason ?? "unknown"})` : post.content}</p>
